...
(use-package consult-mu
  ...
  ;; require extra module for composing (e.g. for interactive attachment)
...
Here is a screenshot of quickly going through some saved searches (using =next-history-element= which is bound to =M-n= by default).
...
*** Attachments
By default, attaching files to emails with mu4e is not very intuitive or interactive. If you use [[https://github.com/jeremy-compostella/org-msg][org-msg]], it provides an interactive command for attaching files, but it is only for one file at a time and for each file you need to go through some menus. Some emacs configs, like [[https://github.com/doomemacs/doomemacs][doomemacs]], do provide their own commands to allow using a dired buffer to attach multiple files to an email (see [[https://github.com/doomemacs/doomemacs/blob/986398504d09e585c7d1a8d73a6394024fe6f164/modules/email/mu4e/autoload/email.el#L272C8-L272C26][+mu4e/attach-files]]). Personally, I prefer using a minibuffer completion (instead of switching to a dired buffer). Therefore =consult-mu-compose-attach= provides an interactive command to attach files to an email. In the minibuffer, you can then call =embark-dwim= on any candidate to add it to attach the file. Note that to add multiple candidates without exiting the minibuffer you either need to set =embark-quit-after-action= to =nil=, or refer to the embark official guide on how to make a no-quit version of the commands here: [[https://github.com/oantolin/embark#quitting-the-minibuffer-after-an-action][embark#quitting-the-minibuffer-after-an-action]]. Similarly, =consult-mu-compose-detach= provides an interactive command to remove already attached files from the message. To use attachment extras, you need to load the compose module by:
...
* Acknowledgments
Obviously this package would not have been possible without the fabulous [[https://github.com/djcb/mu][mu/mu4e]], and [[https://github.com/minad/consult][consult]] packages. It also took inspiration from other packages including but not limited to [[https://github.com/seanfarley/counsel-mu][counsel-mu]], and [[https://github.com/emacsmirror/consult-notmuch][consult-notmuch]].
