...
In the example above, we first run a search *tickets* and sort the results with reverse order (using =-z= option) then narrows down the list of candidates by the word, *concert*.
