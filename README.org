...
It is highly recommended that you also change the preview key binding for =consult-mu-compose= to something other than ='any= because seeing a preview of every file when you are navigating through the minibuffer may be slow and annoying. It's easier to get a preview of the files you are interested in with a key binding like =C-o= than to seea preview of every file.

#+begin_src emacs-lisp
(setq consult-mu-compose-preview-key "C-o")
#+end_src
**** Attaching Files:
The interactive command =consult-mu-compose-attach= provides an interactive command to attach files to an email and it tries its best to provide an intuitive interface. Here are some important notes:
...
(defun embark-dwim-noquit ()
     ...
     (let ((embark-quit-after-action nil))
       (embark-dwim)))
#+end_src
Then you can just attach multiple files to the same message from any folder by using the minibuffer completion. See the screenshot below:
